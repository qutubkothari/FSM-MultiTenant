-- Crescent: Visits by Company / Branch / Salesman (all dates)
-- Includes visit_type.
-- Run in Supabase SQL Editor.

WITH crescent_tenants AS (
  SELECT id, company_name
  FROM tenants
  WHERE is_active = true
    AND company_name ILIKE '%Crescent%'
)
SELECT
  t.company_name,
  COALESCE(
...
FROM visits v
JOIN crescent_tenants t ON t.id = v.tenant_id
