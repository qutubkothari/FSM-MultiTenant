...
-- Add tenant_id to salesman_targets table (if exists)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'salesman_targets') THEN
    ALTER TABLE salesman_targets ADD COLUMN IF NOT EXISTS tenant_id UUID REFERENCES tenants(id) ON DELETE CASCADE;
  END IF;
END $$;
...
-- Update all existing salesman_targets with default tenant (if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'salesman_targets') THEN
    UPDATE salesman_targets SET tenant_id = '00000000-0000-0000-0000-000000000001' WHERE tenant_id IS NULL;
  END IF;
END $$;
...
-- Make tenant_id NOT NULL for salesman_targets (if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'salesman_targets') THEN
    ALTER TABLE salesman_targets ALTER COLUMN tenant_id SET NOT NULL;
  END IF;
END $$;
...
-- Create index for salesman_targets (if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'salesman_targets') THEN
    CREATE INDEX IF NOT EXISTS idx_salesman_targets_tenant_id ON salesman_targets(tenant_id);
  END IF;
END $$;

-- Composite indexes for common queries
CREATE INDEX IF NOT EXISTS idx_users_tenant_phone ON users(tenant_id, phone);
CREATE INDEX IF NOT EXISTS idx_salesmen_tenant_phone ON salesmen(tenant_id, phone);
CREATE INDEX IF NOT EXISTS idx_visits_tenant_salesman ON visits(tenant_id, salesman_id);
...
-- Salesman Targets policies (if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'salesman_targets') THEN
    EXECUTE 'ALTER TABLE salesman_targets ENABLE ROW LEVEL SECURITY';
    
    EXECUTE 'CREATE POLICY "Users can view own tenant targets"
      ON salesman_targets FOR SELECT
      USING (tenant_id = get_user_tenant_id())';
    
    EXECUTE 'CREATE POLICY "Admins can manage own tenant targets"
      ON salesman_targets FOR ALL
      USING (
        tenant_id = get_user_tenant_id() AND
        EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role = ''admin'')
      )';
